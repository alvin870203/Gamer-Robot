...
class TicTacToeGame:
    ...
    def move(self, chess):
        ...
        targetP1 = "550 , 250 , 300.00 , 180.00 , 0.00 ,45" 
        script = "PTP(\"CPP\","+targetP1+",100,0,0,false)"
        send_script(script)

        targetP1 = "550 , 250 , %.2f , 180.00 , 0.00 , 45" %(150+30*i)
        script = "PTP(\"CPP\","+targetP1+",100,0,0,false)"
        send_script(script)
        set_io(0.0)
                        ...
                        for _j in range(-1, 2, 1):
                            if i + _i >= 0 and i + _i <= 2 and line.index(0) + _j >= 0 and line.index(0) + _j <= 2:
                                if self.table[i + _i][line.index(0) + _j] == -1:
                                    swap(i + _i, line.index(0) + _j, i, line.index(0))
                                if white_win() == True:
                                    swap_(i, line.index(0), i + _i, line.index(0) + _j)
                                else:
                                    return
                                

    def upadate_table(self, chess, blue, red):
        for i in range(9):
...
if __name__ == '__main__':
    try:
        ...
        lower_blue = np.array([3, 90, 130])
        higher_blue = np.array([20, 120, 160])
        lower_red = np.array([80, 180, 130])
        higher_red = np.array([120, 230, 180])
        ...
        #     set_io(0.0)
        #     targetP1 = "%f , %f , 300.00 , 180.00 , 0.00 , %.2f" %(Rob[0, i], Rob[1, i], angles[i] - 45)
        #     script = "PTP(\"CPP\","+targetP1+",100,0,0,false)"
        #     send_script(script)

        #     targetP1 = "%f , %f , 150.00 , 180.00 , 0.00 , %.2f" %(Rob[0, i], Rob[1, i], angles[i] - 45)
        #     script = "PTP(\"CPP\","+targetP1+",100,0,0,false)"
        #     send_script(script)
        #     set_io(1.0)
