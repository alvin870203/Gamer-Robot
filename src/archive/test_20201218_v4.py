...
class TicTacToeGame:
    ...
    def move(self):
        set_io(0.0)
        targetP1 = "%f , %f , 300.00 , 180.00 , 0.00 , 135" %(place2rob[self.origin][0], place2rob[self.origin][1])
        script = "PTP(\"CPP\","+targetP1+",100,0,0,false)"
        send_script(script)

        targetP1 = "%f , %f , 150.00 , 180.00 , 0.00 , 135" %(place2rob[self.origin][0], place2rob[self.origin][1])
        script = "PTP(\"CPP\","+targetP1+",100,0,0,false)"
        send_script(script)
        set_io(1.0)
        ...
        targetP1 = "%f , %f , 300.00 , 180.00 , 0.00 , 135" %(place2rob[self.place][0], place2rob[self.place][1])
        script = "PTP(\"CPP\","+targetP1+",100,0,0,false)"
        send_script(script)

        targetP1 = "%f , %f , 150.00 , 180.00 , 0.00 , 135" %(place2rob[self.place][0], place2rob[self.place][1])
        script = "PTP(\"CPP\","+targetP1+",100,0,0,false)"
        send_script(script)
        set_io(0.0)
                        ...
                        for _j in range(-1, 2, 1):
                            if i + _i >= 0 and i + _i <= 2 and line.index(0) + _j >= 0 and line.index(0) + _j <= 2:
                                if self.table[i + _i][line.index(0) + _j] == -1:
                                    ...
                                    self.origin = 3 * (i + _i) + (line.index(0) + _j) + 1
                                    self.place = 3 * (i) + (line.index(0)) + 1
                                    move()
                                    return True
                        ...
                        for _j in range(-1, 2, 1):
                            if line.index(0) + _i >= 0 and line.index(0) + _i <= 2 and i + _j >= 0 and i + _j <= 2:
                                if self.table[line.index(0) + _i][i + _j] == -1:
                                    ...
                                    self.origin = 3 * (line.index(0) + _i) + (i + _j) + 1
                                    self.place = 3 * (line.index(0)) + (i) + 1
                                    move()
                                    return True          
...
def main(counter, chess):
    ...
    while True:
        ...
        lower_blue = np.array([5, 90, 120])
        higher_blue = np.array([10, 140, 210])
        lower_red = np.array([100, 100, 130])
        higher_red = np.array([180, 230, 180])
        ...
        set_io(0.0)
        targetP1 = "%f , %f , 300.00 , 180.00 , 0.00 , 135" %(Rob_blue[0, i]-4, Rob_blue[1, i]+4)
        script = "PTP(\"CPP\","+targetP1+",100,0,0,false)"
        send_script(script)

        targetP1 = "%f , %f , 150.00 , 180.00 , 0.00 , 135" %(Rob_blue[0, i]-4, Rob_blue[1, i]+4)
        script = "PTP(\"CPP\","+targetP1+",100,0,0,false)"
        send_script(script)
        set_io(1.0)
...
if __name__ == '__main__':
    try:
        ...
        T_cm = np.array([[-0.686, 0.7198, 311.5], 
              [0.7198, 0.6957, -234.5], 
              [0, 0, 1]])
        T_mc = np.linalg.inv(T_cm)

        Rob_red = np.matmul(T_mc, cMat_red)
        for i in range(3):
            for j in range(3):
                Rob_red[i, j] = Rob_red[i, j] * 200 / 207
        print('Rob_red', Rob_red)

        Rob_blue = np.matmul(T_mc, cMat_blue)
        for i in range(3):
            for j in range(3):
        ...
        Rob_chess = np.matmul(T_mc, cMat_chess)
        for i in range(3):
            for j in range(9):
